...
def site_list_correction(linked_site):
    if "/" in linked_site:
        ...
        if "gi|CON" in linked_site or "REVERSE" in linked_site:
            return ""
        else:
            return site_correct(linked_site)


# find the site's start and stop line
    ...
    for site in site_pos_orig_dic:
        ...
        if line_up1 == "SameSet":
            ...
            for i in range(site_up, site_down + 1):
                line_list = tab1[i].rstrip("\n").split(",")
                first_cell = line_list[0]
                if first_cell not in ["SameSet", "SubSet"]:
                    tmp.write(tab1[i])
                else:
                    continue
...
def statistic_report_file():
    ...
    for i in range(1, len(rep_table)):
        ...
        if rep_table[i].strip("\n").split("\t")[6] == "Intra":
            intra_num += 1
    # print(intra_num)
    col_dic[6] = float(intra_num) / (float(len(rep_table)) - 1.0)
