import os
os.chdir(r"D:\softwareData\plink2\search_task_2017.11.30.10.02.14_ARGO_CNGP\reports")


def reduce_redundancy_list(list):
    for word in all_site:
...
def get_crosslink_site_info():
    ...
    for fl in filename:
        if fl[-22:-4] == "cross-linked_sites":
            ...
            print(raw_name_list)
            col = [
                "Linked_site", "Total_spec", "Best_svm", "Peptide", "Peptide_mass",
                "Prote_type"
            ]
            for i in range(len(raw_name_list)):
                col.append(str(raw_name_list[i]) + "_spec")
                col.append(str(raw_name_list[i]) + "_E_value")
                    ...
                    for j in range(ordr_pos[l] + 1, ordr_pos[l + 1]):
                        if raw == site_table[j].strip().split(',')[2][:site_table[
                            ...
                            raw_sub_E_value_dic[raw].append(
                                float(site_table[j].strip().split(',')[8]))
