import re, os
wd_dir = r"G:\msData\SLu\G9"
report_path = r"G:\msData\SLu"
# f = open(r"G:\msData\SLu\G1\LuS_NP_output_BS3\reports\pQuant_xl\pQuant.proteins.list").readlines()

titleLine = "Site,d0_spec,d0_best_evalue, d0_svm, d0_pep, d4_spec,d4_best_evalue, d4_svm,d4_pep, d4/d0 ratio, d4/d0 score, d0/d4 ratio,d0/d4 score\n"
...
def merge_d0d4(root_path):
    qt_d0_path = wd_dir + "\\LuS_NP_output_BS3\\reports\\pQuant_xl\\pQuant.proteins.list"
    qt_d4_path = wd_dir + "\\LuS_NP_output_BS3_heavy\\reports\\pQuant_xlink\\pQuant.proteins.list"
    qt_d0_dic = get_quant_dic(qt_d0_path)
    qt_d4_dic = get_quant_dic(qt_d4_path)
    ...
    b = open(os.path.join(report_path, "group%s_xlink.csv" % wd_dir[-1]), 'w')
    b.write(titleLine)
    for site in list(site_set):
        ...
        if site in d0_info_dic and site in d4_info_dic:
            ...
            wlist.extend(qt_d0_dic[site])
            wlist.extend(qt_d4_dic[site])
