...
path = r"G:\msData\20200419\Lacto"  # 搜索fasta 和 PDB 文件所在目录
XL_sites_list = ["K"] # 交联位点

os.chdir(path)

prot_pdb_fasta_dic = {"CNGP":[r"G:\msData\20200419\CNGP\DSS\output\reports\3U28.pdb", r"G:\msData\20200419\CNGP\DSS\output\reports\CNGP.fasta"],
                    "BSA":[r"C:\greenProgram\PyMOL\3v03.pdb", r"D:\FastaDatabase\fasta\bsa.fasta"],
                    "GST":[r"C:\Users\Yong Cao\Documents\pLink\pLink_task_2020.01.16.09.41.51_GST6_025UL\reports\1y6e_GST.pdb", r"D:\FastaDatabase\fasta\GST_sequence.fasta"],
                    "Lacto":[r"F:\Script\pdbDatabase\1fck_Lacto.pdb", r"D:\FastaDatabase\fasta\Lactoferrin.fasta"]
                    }
...
def main_cal_distance(sitesFl_path, pdb_path, fasta_path, repName):
    f = open(sitesFl_path, 'r').readlines()
    pair_list = []
...
if __name__ == "__main__":
    for root, dirs, fls in os.walk(path):
        if root.endswith("reports"):
            prot, linker, output = root.split("\\")[-4:-1]
            pdb_path, fasta_path = prot_pdb_fasta_dic[prot]
            for fl in fls:
                if fl.endswith("v5.csv"):
                    sites_path = os.path.join(root, fl)
            repName = linker+output+".txt"
            main_cal_distance(sites_path, pdb_path, fasta_path, repName)
