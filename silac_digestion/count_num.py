...
for (root, dirs, files) in os.walk("./"):
    for fl in files:
        pdbName = fl[:4]
        linkType = root.split("\\")[-1]
        print(os.path.join(root, fl))
        tlnum = countnum(os.path.join(root, fl), disCufOff)
...
print(repDic)
b = open("report.txt", 'w')
b.write("\t".join(["pdb", "numKK", "numKC", "numKD", "numKE", "numRR", "numKR"])+"\n")
for pdb in repDic:
    if "./KK_60" in repDic[pdb]:
        ...
        numKK = "No"
        print(pdb)
    if "./KC_60" in repDic[pdb]:
        numKC = repDic[pdb]["./KC_60"]
    else:
        numKC = "No"
        print(pdb)
    if "./KD_60" in repDic[pdb]:
        numKD = repDic[pdb]["./KD_60"]
    else:
        numKD = "No"
        print(pdb)
    if "./KE_60" in repDic[pdb]:
        numKE = repDic[pdb]["./KE_60"]
    else:
        numKE = "No"
        print(pdb)
    ...
    wlist = [pdb, numKK, numKC, numKD, numKE, numRR, numKR]
