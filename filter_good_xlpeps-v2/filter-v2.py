...
for i in range(1,len(lines)):
    ...
    if pp in ans:
        ans[pp].append([mz,ms2_rt,svmscore,title,ms1_rtmin,ms1_rtmax,ms1_scans,ms1_mono_int,ms1_rts])
    else:
        ans[pp]=[[mz,ms2_rt,svmscore,title,ms1_rtmin,ms1_rtmax,ms1_scans,ms1_mono_int,ms1_rts]]


fout=open(inclusion_list,'w')
fout.write('pep_pair,charge,mods,theo_m/z,id_rt_min,id_rt_max,spec_cnt,svm_best,svm_worst,pq_rt_min,pq_rt_max,fit_chrom_mean,fit_chrom_std,mean-3std,mean+3std\n')

fout2=open(inclusion_list_detail,'w')
fout2.write('pep_pair,charge,mods,theo_m/z,id_rt_min,id_rt_max,spec_cnt,svm_best,svm_worst,pq_rt_min,pq_rt_max,fit_chrom_mean,fit_chrom_std,mean-3std,mean+3std\n')
...
for k,v in ans.items():
    ...
    svm_best=min(v,key=lambda x:x[2])[2]
    svm_worst=max(v,key=lambda x:x[2])[2]
    
    specn=len(v)
    
    ms1_rtmin=min(v,key=lambda x:x[4])[4]
    ms1_rtmax=max(v,key=lambda x:x[5])[5]
    ...
    fout.write('%s,%s,%s,%f,%f,%f,%d,%E,%E,%f,%f,%f,%f,%f,%f\n'%(pep,charge,mod,theo_mz,id_rtmin,id_rtmax,specn,svm_best,svm_worst,ms1_rtmin,ms1_rtmax,mean,std,mean-3*std,mean+3*std))
    
        
    fout2.write('%s,%s,%s,%f,%f,%f,%d,%E,%E,%f,%f,%f,%f,%f,%f\n'%(pep,charge,mod,theo_mz,id_rtmin,id_rtmax,specn,svm_best,svm_worst,ms1_rtmin,ms1_rtmax,mean,std,mean-3*std,mean+3*std))
