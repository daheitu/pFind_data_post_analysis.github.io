...
def split_seq(seq, a):
    seq_list_1 = seq.split(a)
    new_list = []
    for i in range(len(seq_list_1)-1):
        new_list.append(seq_list_1[i])
        new_list.append(a)
    new_list.append(seq_list_1[-1])
    return new_list


def get_peps(piecelist, miss_cleav):
    m = miss_cleav
    final_pep = []
...
def getall_peps(piecelist, max_mis):
    rep_list = [piecelist]
    for i in range(1, max_mis+1):
        cur_list = get_peps(piecelist, i)
        rep_list.append(cur_list)
    return rep_list


def writePep2file(rep_list, min_len, max_len, seq, pname, b):
...
def main():
    ...
    for pname in fasta_dic:
        ...
        for i in range(len(full_list)):
            if "R" in full_list[i]:
                full_list[i] = split_seq(full_list[i], 'R')

        final = []
        for pep in full_list:
            if type(pep) != list:
                final.append(pep)
            else:
                for subpep in pep:
        ...
        rep_list = getall_peps(final, max_misclavage)
        writePep2file(rep_list, min_pep_len, max_pep_len, seq, pname, b)
    b.close()
