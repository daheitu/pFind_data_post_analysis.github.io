...
AA_dict = dict(HIS="H", MET="M", THR="T", PHE="F", PRO="P", SER="S", TRP="W",
               ...
               DPR="Dpr", MSE="Mse", MLZ="Mlz",TPO="Tpo", UNK="Uuk", SMC="Smc")
...
for name in filename:
    if name[-4:] == ".pdb":
        ...
        for chain in chains:
                ...
                if line[:6] == "SEQRES" and line[11] == chain:
                    ...
                    if chain_type == "Protein":
                        for i in range(element_num):
                            raw_seq.append(AA_dict[sequence[i*4:(i+1)*4].strip()])
                    else:
                        for i in range(element_num):
                            if sequence[i * 4:(i + 1) * 4].strip():
                                raw_seq.append(sequence[i*4:(i+1)*4].strip())
                    vars()["chain" + chain + "_seq_list"].append("".join(raw_seq))
                elif line[:4] == "ATOM":
                    break
            ...
            chain_info = ",".join([chain, str(chain_length_pdb_dic[chain]), chain_type_pdb_dic[chain]])
