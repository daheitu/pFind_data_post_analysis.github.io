...
# path = r"G:\msData\synthetic_pepteide_rawdata\CV3_CV7\DSSO\output_score"

# pLink2_id_folder = path
# inclusion_list = os.path.join(path, 'inclusion_list1.csv')
# inclusion_list_detail = os.path.join(path, 'inclusion_list_detail1.csv')
...
def get_basic_info(pLink2_id_folder):
    ...
    for c in reports:
        if c.endswith("cross-linked_spectra_filter.csv"):
            specid = r'%s\reports\%s' % (pLink2_id_folder, c)
            specid_list.append(specid)
    ...
    return specid_list, pq, specs, ms1specs
...
def cal_inclu_time(specid, inclusion_list, inclusion_list_detail, specs, ms1specs, pq):
...
def cal_write_result(pLink2_id_folder):
    specid_list, pq, specs, ms1specs = get_basic_info(pLink2_id_folder)
    path = pLink2_id_folder
    if len(specid_list) == 1:
        ...
        for specid in specid_list:
            cal_inclu_time(specid, inclusion_list, inclusion_list_detail, specs, ms1specs, pq)
    else:
        for specid in specid_list:
            bsName = os.path.basename(specid)
            raw_name = bsName[:bsName.find('cross-linked_spectra_filter.csv')]
            inclusion_list = os.path.join(path, raw_name + 'inclusion_list.csv')
            inclusion_list_detail = os.path.join(path, raw_name + 'inclusion_list_detail.csv')
            cal_inclu_time(specid, inclusion_list, inclusion_list_detail, specs, ms1specs, pq)


if __name__ == "__main__":
    print("Calculating")
    pLink2_id_folder = r"G:\msData\synthetic_pepteide_rawdata\CV3_CV7\DSSO\output_score"
    cal_write_result(pLink2_id_folder)
